...
function ProfileCard() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(
      `https://bip-backend.fly.dev/api/users/change/${user.id}`,
      {
        method: "PATCH",
        headers: {
    ...
    if (msg == "Valid Player") {
      auth.updateUser({
        ...user,
        name: currUser.name,
        player_id: currUser.player_id,
        team_id: currUser.team_id,
        team: currUser.team_name,
      });
        ...
        <section className={styles.inline_box}>
          ...
          <section className={styles.info_box}>
            ...
            <section className={styles.btn_section}>
              <button className="button_light" onClick={callLogout}>
                Logout
              </button>
              {!editingProf && (
                <button className="button_light" onClick={toggleForm}>
                  Edit Account Info
                </button>
              )}
              {editingProf && (
